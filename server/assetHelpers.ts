import { SUB_TO_MAIN_MAP } from "../src/lib/assetCatalogByType.js";
...
export function mapSheetRow(item: Record<string, unknown>): MappedAsset {
  let mainCategory = getVal(item, ["Category", "Main Category", "mainCategory"]);
  const subCategory = getVal(item, ["Sub Category", "Asset Type", "Type", "subCategory"]);
  
  // Auto-healing for old/broken records that default to IT Assets
  if (!mainCategory || mainCategory === "IT Assets") {
    if (subCategory && SUB_TO_MAIN_MAP[subCategory]) {
      mainCategory = SUB_TO_MAIN_MAP[subCategory];
    } else {
      mainCategory = mainCategory || "IT Assets";
    }
  }
  ...
  if (mainCategory === "IT Assets" && subCategory) {
    ...
    if (tLower === "laptop" || tLower === "desktop" || tLower === "laptop kit" || tLower === "laptop / desktop") {
  ...
  return {
    ...
    warrantyEndDate: getVal(item, ["Warranty Date", "Warranty Expiry Date", "Warranty End"]),
    ram: getVal(item, ["RAM"]),
    ssd: getVal(item, ["SSD", "Storage"]),
    cpu: getVal(item, ["CPU", "Processor"]),
    windowsVersion: getVal(item, ["Windows Version", "OS"]),
    assetType: getVal(item, ["Asset Type", "Type"]) || mappedSubCategory || "Laptop",
    ...
    documentUrl: getVal(item, ["Document URL", "Document URL / Attached Documents", "Document Link", "Document"]),
...
export async function fetchAllAssets(gasWebappUrl: string, dbMode?: string): Promise<MappedAsset[]> {
  const url = dbMode === "redesigned" ? `${gasWebappUrl}${gasWebappUrl.includes("?") ? "&" : "?"}action=list_assets_redesigned` : gasWebappUrl;
  const response = await fetch(url);
  const text = await response.text();
  const parsed = JSON.parse(text);
  const data = Array.isArray(parsed) ? parsed : (parsed.assets || []);

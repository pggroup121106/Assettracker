...
const USERS_GAS_TIMEOUT_MS = 20000;
...
export async function fetchUsersFromGas(
  ...
  const payloads: Record<string, unknown>[] = [{ action: "list_users" }];

  const postResults = await Promise.allSettled(
    payloads.map((payload) =>
      proxyToGas(payload, 12000).then(parseUsersFromGasResult)
    )
  );
  ...
  const getResults = await Promise.allSettled(
    queries.map(async (q) => {
      ...
      try {
        const res = await fetch(`${gasWebappUrl}?action=${q}`, {
          signal: controller.signal,
        });
        const text = await res.text();
        return parseUsersFromGasResult(JSON.parse(text));
      } finally {
        clearTimeout(timer);
      }
    })
...
export async function getAllUsers(
  ...
  if (spreadsheetId && usersSheetGid && listFromGoogleApi) {
    try {
  ...
  if (sheetUsers.length === 0 && gasWebappUrl) {
    try {

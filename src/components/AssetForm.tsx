...
import { Monitor, User, Cpu, MapPin, LayoutDashboard, ChevronLeft, ChevronRight, Package, Building2, Keyboard, Mouse, Zap } from "lucide-react";
...
import { MAIN_CATEGORIES, CATEGORY_SUBCATEGORIES, PERIPHERAL_TYPES } from "../lib/assetCatalogByType";
...
const PRIMARY_TYPES = ['Laptop', 'Desktop', 'Input/Output Device'];
...
export default function AssetForm({ initialData, onSubmit, onCancel, loading, layout = "modal" }: AssetFormProps) {
  ...
  /** Reload all fields when opening a different asset for edit. */
  useEffect(() => {
    if (!initialData?.id) return;
    const fd = assetToFormData(initialData);
    setFormData(fd);
    setDynamicDetails(fd.dynamicDetails || {});
    lastResolvedTypeIdRef.current = fd.assetTypeId || null;
    setFormStep(2);
    setFieldErrors({});
    setMacError(null);
  ...
  const getSubCategories = (mainCategory: string) => {
    ...
    return Array.from(new Set([...staticSubs, ...customSubs]));
  };
  ...
  const entryProfile = React.useMemo(
    () => getEntryFormProfile(formData, activeTypeDef),
    [formData, activeTypeDef]
  );
  ...
  const previewUrl = getDeviceImageUrl(formData.imageUrl) || getAssetPreviewUrl(formData.mainCategory || "IT Assets", formData.subCategory || "", formData.assetType);

  const selectAssetType = (newType: AssetType) => {
    const brands = getBrandListForAssetType(catalog, newType);
    const make = brands.includes(formData.make) ? formData.make : "";
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 2) {
      ...
      if (entryProfile.useBrandModelFields && (!formData.make?.trim() || !formData.model?.trim())) {
        toast.error("Brand and model are required");
        return false;
      }
      if (!formData.serialNumber?.trim() || !formData.assetCode?.trim()) {
        toast.error(`${entryProfile.serialLabel} and ${entryProfile.assetCodeLabel} are required`);
        return false;
      }
      if (fieldErrors.serialNumber || fieldErrors.assetCode) {
  ...
  }, [formData.serialNumber, formData.assetCode, formData.macAddress, initialData?.id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const dataToSubmit = {
      ...
      status: formData.maintenanceRequired === 'Yes' ? 'Under Maintenance' : (formData.status || 'Available'),
    ...
    try {
      // If editing an existing asset, submit directly without step navigation
      if (initialData?.id) {
        ...
        const codeOk = await checkUnique("assetCode", dataToSubmit.assetCode);
    ...
    >
      ...
      <nav className="flex items-center gap-2 p-1 bg-slate-100 rounded-2xl border border-slate-200">
        {STEPS.map(({ n, title, icon: Icon }) => (
          <button
            key={n}
            type="button"
            onClick={() => n < formStep || validateStep(formStep) ? setFormStep(n) : undefined}
            className={cn(
        ...
        <div className="rounded-2xl overflow-hidden border border-slate-200 bg-white shadow-sm">
          <div className="grid md:grid-cols-5 gap-0">
            <div className="md:col-span-2 p-6 flex flex-col justify-center bg-gradient-to-br from-slate-900 to-slate-800 text-white">
              <img src="/logo.png" alt="AssetVault" className="w-12 h-12 object-contain mb-4" />
            ...
            <div className="md:col-span-3 relative min-h-[280px] bg-slate-50 flex items-center justify-center p-6">
              <img
                src={previewUrl}
                alt={formData.subCategory || formData.assetType}
              ...
              <div className="absolute bottom-0 inset-x-0 p-4 bg-gradient-to-t from-slate-900/80 to-transparent pointer-events-none">
                ...
                <p className="text-white/80 text-xs">{formData.subCategory || formData.assetType}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            ...
            {allowedCategories.length > 1 ? (
              <select
                ...
                onChange={(e) => {
                  ...
                  const firstSub = subs[0] || "";
                  setFormData((prev) => applyCategorySelection(prev, mainCat, firstSub, typeConfig));
                  setDynamicFieldErrors({});
                  setMacError(null);
          ...
          {(formData.mainCategory || "IT Assets") === "IT Assets" ? (
            <div className="space-y-4 pt-2">
              ...
              <div className="flex flex-col sm:flex-row gap-1 bg-slate-100 p-1 rounded-xl w-full border border-slate-200">
                {PRIMARY_TYPES.map(type => {
                  const isPeripheralGroup = type === 'Input/Output Device';
                  const isSelected = isPeripheralGroup
                    ? PERIPHERAL_TYPES.includes(formData.assetType)
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        let subCat = "Other IT Asset";
                        if (newType === "Laptop" || newType === "Desktop") subCat = "Laptop / Desktop";
                        else if (newType === "Keyboard" || newType === "Mouse" || newType === "QR Scanner") subCat = "Input Device";
                        else if (newType === "Monitor") subCat = "Output Device";
                        else if (["Network Switch", "Network Rack", "Access Point", "Firewall", "Network Controller"].includes(newType)) subCat = "Network Device";
                        else if (newType === "External HDD") subCat = "Storage Device";
                        else if (newType === "Printer") subCat = "Printer / Scanner";
                        else if (newType === "Camera" || newType === "NVR") subCat = "CCTV / Security Device";
                        else if (newType === "UPS") subCat = "Server / UPS";
                    ...
                    {PERIPHERAL_TYPES.map(pType => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          let subCat = "Other IT Asset";
                          if (pType === "Laptop" || pType === "Desktop") subCat = "Laptop / Desktop";
                          else if (pType === "Keyboard" || pType === "Mouse" || pType === "QR Scanner") subCat = "Input Device";
                          else if (pType === "Monitor") subCat = "Output Device";
                          else if (["Network Switch", "Network Rack", "Access Point", "Firewall", "Network Controller"].includes(pType)) subCat = "Network Device";
                          else if (pType === "External HDD") subCat = "Storage Device";
                          else if (pType === "Printer") subCat = "Printer / Scanner";
                          else if (pType === "Camera" || pType === "NVR") subCat = "CCTV / Security Device";
                          else if (pType === "UPS") subCat = "Server / UPS";
      ...
      <section className="space-y-6">
        <div className="flex items-center gap-4 p-4 bg-blue-50 border border-blue-100 rounded-2xl">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600">{formData.subCategory || formData.assetType}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-5">
          {entryProfile.useAssetNameField && (
            <div className="space-y-1.5">
              <label className="label-caps text-red-500">Asset Name *</label>
              <input
                required
                name="assetName"
                ...
                placeholder={
                  entryProfile.mainCategory === "Vehicle Assets"
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 space-y-1.5">
              <label className="label-caps text-red-500">{entryProfile.serialLabel} *</label>
              <input
                required
                name="serialNumber"
                value={formData.serialNumber}
                onChange={handleChange}
                placeholder="ID-99201-SYS"
                className={cn(
            ...
            <div className="flex-1 space-y-1.5">
              <label className="label-caps text-red-500">{entryProfile.assetCodeLabel} *</label>
              <input
                required
                name="assetCode"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <SmartSelect
              ...
              options={optionsWithValue(catalog.vendors ?? [], formData.vendorName)}
              onChange={(vendorName) => setFormData((prev) => ({ ...prev, vendorName }))}
              onAddCustom={(v) => persistCatalog((c) => addVendor(c, v))}
              onDeleteOption={(v) => persistCatalog((c) => removeVendor(c, v))}
            />
          ...
          <div className="space-y-1.5">
            ...
            >
              ...
              <option value="EXISTING ASSETS">EXISTING ASSETS</option>
              <option value="Average">Average</option>
              <option value="Poor">Poor</option>
              <option value="Damaged">Damaged</option>
              <option value="NEW MAINTAINANCE">NEW MAINTAINANCE</option>
            </select>
          </div>

          {entryProfile.requireMacAddress && (
            <div className="space-y-1.5 font-mono p-4 bg-slate-50 rounded-xl border border-slate-200">
            ...
            {!showAmc ? (
              ...
              <div className="p-5 bg-blue-50/30 rounded-2xl border border-blue-100 space-y-4 relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="label-caps text-slate-400">Asset Name</label>
                    <input
                      disabled
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <SmartSelect
                label="RAM"
                value={formData.ram}
                options={optionsWithValue([...RAM_OPTIONS, ...(catalog.ram || [])], formData.ram)}
            ...
            <div className="space-y-1.5">
              <SmartSelect
                label="Storage (SSD)"
                value={formData.ssd}
                options={optionsWithValue([...SSD_OPTIONS, ...(catalog.ssd || [])], formData.ssd)}
            ...
            <div className="space-y-1.5">
              <SmartSelect
                ...
                required
                value={formData.cpu}
                options={optionsWithValue([...CPU_OPTIONS, ...(catalog.cpu || [])], formData.cpu)}
            ...
            <div className="space-y-1.5">
              <SmartSelect
                ...
                options={optionsWithValue([...WINDOWS_OPTIONS, ...(catalog.windowsVersion || [])], formData.windowsVersion)}
      ...
      <section className="space-y-6">
        <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl flex items-center gap-3">
          ...
          <div>
            ...
            <h4 className="font-black text-slate-900">Location, Plant & Assignee</h4>
          </div>
        </div>
          ...
          {formData.location && (
            <div className="space-y-1.5">
              <SmartSelect
                ...
                options={plantsForLocation.map((p) => p.code)}
                onChange={(plantCode) => setFormData((prev) => ({ ...prev, plantCode }))}
          ...
          {isFieldEnabled('department') && (
            <div className="md:col-span-2">
              <SmartSelect
                ...
                options={optionsWithValue(catalog.departments ?? [], formData.department)}
                onChange={(department) => setFormData((prev) => ({ ...prev, department }))}
                onAddCustom={(d) => persistCatalog((c) => addDepartment(c, d))}
                onDeleteOption={(d) => persistCatalog((c) => removeDepartment(c, d))}
              />
            </div>
          )}
        </div>
      ...
      <section className="space-y-5 border-t border-slate-100 pt-6">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5 font-mono">
              <label className="label-caps">Return Date</label>
              <input
                type="date"
                name="returnDate"
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-1.5 font-mono">
              <label className="label-caps">Extra Items (Remarks)</label>
              <input
                type="text"
                name="extraItems"
                value={formData.extraItems || ""}
                onChange={handleChange}
                placeholder="Any extra items provided"
                className="w-full input-geometric"
              />
            </div>
            <div className="space-y-1.5 font-mono">
              <label className="label-caps">Missing Items (Remarks)</label>
              <input
                type="text"
                name="missingItems"
                value={formData.missingItems || ""}
                onChange={handleChange}
                placeholder="Any missing items reported"
                className="w-full input-geometric"
              />
            </div>
          </div>
      ...
      <div className="flex gap-3 justify-between pt-6 border-t border-slate-100">
        ...
        <div className="flex gap-3">
          ...
          {formStep < 3 ? (
            <button type="button" onClick={goNext} disabled={isSubmitting} className="btn-primary-geometric flex items-center gap-2 disabled:opacity-50">
              Next <ChevronRight size={16} />
            </button>
          ) : (
            <button type="submit" disabled={isSubmitting || loading} className="btn-primary-geometric">

...
export function addVendor(catalog: AssetCatalog, vendor: string): AssetCatalog {
  const v = vendor.trim();
  if (!v || catalog.vendors.includes(v)) return catalog;
  return { ...catalog, vendors: [...catalog.vendors, v].sort() };
}
...
export function removeVendor(catalog: AssetCatalog, vendor: string): AssetCatalog {
  const v = vendor.trim();
  const deleted = { ...(catalog.deletedOptions || {}) };
  if (!deleted.vendors) deleted.vendors = [];
  if (!deleted.vendors.includes(v)) deleted.vendors.push(v);
  const vendors = (catalog.vendors || []).filter((x) => x !== v);
  return { ...catalog, vendors, deletedOptions: deleted };
}
...
export function removeSubCategory(catalog: AssetCatalog, mainCat: string, subCat: string): AssetCatalog {
  ...
  return { ...catalog, subCategories, deletedOptions: deleted };
}

...
import { Plus, Trash2, MapPin, Building2, List, Layers, Database, Edit2, Check, X, Archive, AlertTriangle } from 'lucide-react';
...
type Tab = 'locations' | 'plants' | 'fields' | 'types' | 'database';
...
export default function SettingsPage() {
  ...
  const saveSettings = async (next: AppSettings, actionMsg: string = 'Saving...') => {
    ...
    try {
      ...
      if (data.sheetWarning) {
        ...
        toast.success('Saved to Database (Locations & Plants sheets)');
      }
    } catch (err: any) {
      toast.error(err.message);
    } finally {
  ...
  const tabs = [
    ...
    if (t.id === 'types' || t.id === 'database') return isItAdmin;
    return true;
  });
    ...
    <div className="flex-1 overflow-auto p-8 bg-slate-50">
      <div className={tab === 'types' ? 'max-w-6xl mx-auto' : 'max-w-3xl mx-auto'}>
        ...
        <p className="text-sm text-slate-500 mb-6">
          Manage locations, plant codes, custom form fields, asset types config and main database setup.
        </p>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
          ...
          {tab === 'database' && isItAdmin && <DatabaseSetupPanel />}
        </div>
      </div>
